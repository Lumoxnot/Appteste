...
import { Cpu, Search, Wrench, DollarSign, ShieldCheck, Camera, Clock } from 'lucide-react';
...
export default function Acompanhar() {
  ...
  const [codigo, setCodigo] = useState(osId || '');
  const [os, setOs] = useState(null);
  const [loading, setLoading] = useState(false);
  const [searched, setSearched] = useState(false);

  async function buscar(numero) {
    const valor = (numero ?? codigo).trim();
    if (!valor) return;
    setLoading(true);
    setSearched(true);
    try {
      const results = await base44.entities.OrdemServico.filter(
        { numero: valor },
        '-created_date',
        5
      );
      setOs(results.length > 0 ? results[0] : null);
    } catch (err) {
        ...
        {searched && !loading && !os && (
          <Card className="glass-card border-border p-8 text-center">
            <p className="text-sm text-muted-foreground">
              Nenhuma ordem de serviço encontrada com o código <span className="text-white font-medium">{codigo}</span>
            </p>
          </Card>
        )}
